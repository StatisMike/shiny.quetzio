...
For long surveys and frequently used ShinyApps
I recommend generating the objects beforehand, saving them with `saveRDS()` or
`save` in the app directory and then loading them with `readRDS()` or `load()`
to cut the time for initializing the object during the loadtime.
