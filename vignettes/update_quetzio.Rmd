...
In this vignette the usage of these features in `quetzio_server` will be
...
- and code for questionnaire generation:
    ...
    demo_quetzio <- quetzio_server$new(
      source_method = "yaml",
...
after triggering the *observeEvent* the values will be filled.
    ...
    observeEvent(input$update, {
      # call the method in your more complicated questionnaire,
      # in this example it is assigned to 'complex_quetzio'
      complex_quetzio$update_values(
...
- We need to create additional source file telling the *quetzio_server* which
...
>If the value is not represented in this label update source, then when reactive
passed to the 'trigger' contains that value, the default labels will be shown.

- When everything is set, we can then create our questionnaire and its
reactivity within the server. It can be done by utilizing the *update_labels*
method of *quetzio_server*
    ...
    # call the method passing the reactive as a trigger
...
within the *quetzio_server$answers()* only after the questionnaire submission!
