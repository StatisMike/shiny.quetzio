...
vignette: >
  %\VignetteIndexEntry{Create questionnaire}
...
Package **shiny.survey** provides a relatively simple but also highly
...
It is main element of the whole functionality of **shiny.survey**. Its the 
...
```{r quetzio_server_logic, echo=F, fig.cap="Logic of quetzio_server object"}
...
- **inputId** (as the name of the sequence)
- **type**
- **label**
...
df_source <- data.frame(
  ...
  chr_placeholder = c("Write inside of me, please!", NA),
...
There are some more features that are out of the scope of this vignette, for more
information check other vignettes and help files

- Functional features
  - get answers in form of data.frame with **get_answers_df** method
  - toggle the UI to be visible or not with **toggle_ui** method
  - add instructions and additional item descriptions during initialization
  with **desc_yaml**, **desc_gsheet_id** and **desc_gsheet_sheetname** or
  **desc_object**
...
## Alternatives to shiny.survey

I feel like it needs to be stated that **shiny.survey** isn't your only option for
...
Main difference between these are the embedding - while **shiny.surveys** can be used
to create questionnaire (or multiple of them) within more complex Shiny application, 
the **shinysurverys** are tailored to create whole ShinyApp by itself. They also
currently (as of 0.2.0.) don't offer built-in support with **googlesheets4**, 
either for source or data collection.
