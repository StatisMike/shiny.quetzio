...
- **quetzio_link$message()** list of *message()* of individual questionnaires
...
There are also additional methods to object of class *quetzio_link_server* - they
are mostly analogous to methods of *quetzio_server*, but with some additional tweaks.
To learn about these methods read **Update quetzio_server object** (WIP)

### Reactive label update

You can update labels of any of linked questionnaires reactively with *update_labels*
method. It works basically the same like in method for *quetzio_server*, but you
...
# after that - just make a call in your server to the method
quetzio_link$update_labels(
  quetzio_name = "gender", # specify the name of questionnare
  tigger = gender_react,   # pass reactive (without parentheses!)
...
Method is mostly identical to `quetzio_server$update_values()`, though you
need to also provide `quetzio_name` argument to choose which questionnaire
